...
<?php foreach($list as $num_row => $row){ ?>
    <tr class="cole-row"> <!-- ricardo  para hacer click a la columna y ejecutar algun evento -->
        <!-- Ricardo: Dato de campos-->
        <?php foreach($columns as $column){?>
            <td >
                <?php echo $row->{$column->field_name} != '' ? $row->{$column->field_name} : '&nbsp;' ; ?>
            </td>
        <?php }?>
        <!-- Ricardo: Dato de campos  FIN-->
                    ...
                    <?php if (!empty($row->action_urls) || !$unset_read || !$unset_delete) { ?>
                        ...
                        <?php if ($show_more_button) { ?>
                            <div class="btn-group dropdown">
                                <a Xtype="button" class="btn dropdown-toggle" data-toggle="dropdown">
                                    <?php // echo $more_string; ?>
                                    <span class="contentSmallNormal glyphicon glyphicon-menu-hamburger"></span>
                                </a>
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php if (!$unset_read) { ?>
                                        <li>
                                            <a href="<?php echo $row->read_url?>"><i class="Xfa Xfa-eye"></i> <?php echo $this->l('list_view')?></a>
                                        </li>
                                    <?php } ?>
                                    <?php if(!$unset_edit){?>
                                         <li>
                                            <a href="<?php echo $row->edit_url?>"><i class="Xfa Xfa-pencil"></i> <?php echo $this->l('list_edit'); ?></a>
                                         </li>
                                    <?php } ?>
                                    <?php if (!$unset_delete) { ?>
                                        <li>
                                            <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row">
                                                <i class="Xfa Xfa-trash-o Xtext-danger"></i>
                                                <span class="Xtext-danger"><?php echo $this->l('list_delete')?></span>
                                            </a>
                                        </li>
                                    <?php } ?>
                                </ul>
